...
module BBS2ch
  class ReadThreadAction
    ...
    def execute
      ...
      cache = @cache_manager.load(index, cache_picker(delta, index))
      option = @option_manager.load()
      thread_content = TwoB::Thread.new(self, cache, delta, @requested_picker, option)
      ...
      @thread_key.read_counter.update(@thread_key.number, thread_content.res_count)
